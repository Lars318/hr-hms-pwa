...
export const departmentRouter = router({
  ...
  // HR/ADMIN: oppdater avdelingsnavn
  update: hrProcedure
    .input(z.object({ id: z.string(), name: z.string().min(1).max(100) }))
    .mutation(async ({ ctx, input }) => {
      const exists = await ctx.db.department.findUnique({ where: { id: input.id } });
      if (!exists) throw new TRPCError({ code: "NOT_FOUND", message: "Avdeling ikke funnet." });
      return ctx.db.department.update({ where: { id: input.id }, data: { name: input.name } });
    }),

    ...
    .mutation(async ({ ctx, input }) => {
      if (process.env.EMAIL_NOTIFICATIONS_ENABLED !== "true") {
        throw new TRPCError({
          ...
          message: "E-post er ikke aktivert. Sett EMAIL_NOTIFICATIONS_ENABLED=true (+ RESEND_API_KEY og verifisert avsenderdomene) i Vercel for å sende invitasjoner.",
        });
      }
      ...
      for (const p of people) {
        try {
          ...
          await sendEmail({
            to: p.email,
            subject: "Velkommen til Truls HR — sett passordet ditt",
            text: `Hei ${firstName}!\n\nDu har fått tilgang til Truls HR — Pulsfollo sin HR- og HMS-app.\n\nSett passordet ditt her:\n${link}\n\nEtter at du har satt passord kan du logge inn på ${appUrl}.\n\nHilsen Pulsfollo`,
            html: `
<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:520px;margin:0 auto">
  <div style="background:#456237;color:#fff;padding:24px;border-radius:16px 16px 0 0">
    <h1 style="margin:0;font-size:20px">Velkommen til Truls HR</h1>
  </div>
  <div style="border:1px solid #eee;border-top:0;padding:24px;border-radius:0 0 16px 16px">
    <p>Hei ${firstName}!</p>
    <p>Du har fått tilgang til <strong>Truls HR</strong> — Pulsfollo sin app for HR og HMS (fravær, avvik, dokumenter og mer).</p>
    ...
    <p style="font-size:13px;color:#666">Etterpå kan du logge inn på <a href="${appUrl}">${appUrl}</a> og installere appen på mobilen.</p>
          ...
          await ctx.db.profile.update({ where: { id: p.id }, data: { invitedAt: new Date() } });
          results.push({ email: p.email, status: "sent" });
        } catch (e) {
          results.push({ email: p.email, status: "error", message: e instanceof Error ? e.message : "Ukjent feil" });
        }
      }
